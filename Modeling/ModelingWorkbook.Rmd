...
tmpDF <- cbind(speed_zone = row.names(accidentTable), as.data.frame.matrix(accidentTable))

tmpDF %<>% mutate( percent_fatal = (fatal/(fatal+non_fatal))*100 ) %>% arrange(-percent_fatal)
...
accidentTableToDF <-function(accidentTable) {
  colnames(accidentTable) <- c("non_fatal", "fatal")
...
print(accidentTableToDF(accidentTable))

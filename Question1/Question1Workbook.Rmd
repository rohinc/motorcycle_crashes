...
#setupPackage("polr")  #  ordinal logistic regression - not available for R 4.0.2 :-(
setupPackage("ordinal")
...
Would have liked to have used the polr package but not available for this version of R, so will use the oridnal package and it's function for cumulative link models for ordinal regresion (clm).
...
Preicitons are all hospitalisation!
...
# TODO: have to come back and look at how to do the following calc.
#print(paste("Model 3 accuracy:", sum(as.numeric(predictions)==as.numeric(testDF$Crash_Severity_Fac))/nrow(testDF)))
```
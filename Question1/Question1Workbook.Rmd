...
# following isn't quite right, as it somes up the rainfall for each accident - so if two accidents roughly same
# time in same area the rainfall will be doubled up - also doesn't include rainfall for days with no 
#  accidents ..
...
ggplot() +
  geom_bar(aes(x=feat_counts$Crash_Month,
               y=feat_counts$fatalities*max(feat_counts$rainfall)/max(feat_counts$fatalities)),  
           stat="identity") +
  geom_line(aes(x=feat_counts$Crash_Month, y=feat_counts$rainfall, group=1), colour="blue") +
  xlab("Month") +
  scale_y_continuous("Avg. Fatalities",
                     sec.axis = sec_axis(~., name="Avg. Rainfall") ) +
  ggtitle(paste("Avg Fatalities and rainfall ", min(feat_counts$Crash_Year), " to ",  max(feat_counts$Crash_Year))) +
